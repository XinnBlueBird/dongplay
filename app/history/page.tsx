    ...
    <div className="max-w-4xl mx-auto px-4 py-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-[#94a3b8]">{history.length} watched</p>
        </div>
        {history.length > 0 && (
      ...
      {history.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h3 className="text-lg font-bold text-[#94a3b8] mb-2">No watch history</h3>
          <p className="text-sm text-[#64748b]">Start watching donghua and your history will appear here.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {history.map((item) => (
            <Link key={item.slug + item.timestamp} href={`/watch/${item.slug}`} className="flex items-center gap-4 p-3 rounded-xl bg-[#12121a] border border-[#1e1e2e] hover:border-[#6366f1]/30 transition-colors group">
              ...
              <div className="flex-1 min-w-0">
                ...
                {item.episode > 0 && <p className="text-xs text-[#64748b]">Episode {item.episode}</p>}
              </div>
